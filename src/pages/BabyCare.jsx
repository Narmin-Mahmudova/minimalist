...
import productsData from '../data/babycare.json';
...
function ProductCard({ product, index, isInWishlist, addToWishlist, addToCart, navigate }) {
  ...
  return (
    <div
      ...
      className={`bg-white cursor-pointer transition-all duration-700 ease-out transform ${
    ...
    >
      <div className="relative bg-gray-200 aspect-[3/4] mb-4 overflow-hidden">
        ...
        <img src={product.image} alt={product.name} className="w-full h-full object-contain p-6 block" />
      </div>

      <div>
        <h3 className="text-base font-semibold text-gray-900 mb-1">{product.name}</h3>
        <p className="text-sm text-gray-400 mb-2">{product.concern}</p>
        <p className="text-base font-semibold text-gray-900 mb-0.5">₹ {product.price}</p>
        <p className="text-sm text-gray-400 line-through mb-3">₹ {product.oldPrice}</p>
        <button 
          className="w-full py-4 bg-gray-900 text-white border-0 text-sm font-medium cursor-pointer transition-colors duration-300 hover:bg-gray-800"
...
function BabyCare() {
  ...
  const addToWishlist = (product) => {
    toggleWishlist({
      id: product.id,
      name: product.name,
      description: product.concern,
      price: product.price,
      image: product.image
    });
  };

  const { categories, steps, types, concerns, ingredients, availability } = filtersData.babycare;
  ...
  return (
    <div className="min-h-screen bg-white">
      <div 
        ref={headerRef} 
        className={`py-12 text-center transition-all duration-700 transform ${
          isHeaderVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-6'
        }`}
      >
        <div className="max-w-7xl mx-auto px-5">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Baby Care</h1>
          <p className="text-base text-gray-500 max-w-[700px] mx-auto leading-relaxed">
