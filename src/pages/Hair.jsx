...
import productsData from '../data/hair.json';
import useScrollAnimation from '../hooks/useScrollAnimation';
import { useCart } from '../context/CartContext';
import { useWishlist } from '../context/WishlistContext';

function ProductCard({ product, isInWishlist, addToWishlist, addToCart, navigate }) {
  const [cardRef, isCardVisible] = useScrollAnimation();

  return (
    <div
      ref={cardRef}
      className={`bg-white flex flex-col h-full cursor-pointer transition-all duration-700 ease-out ${
    ...
    >
      <div className="relative bg-gray-200 aspect-[3/4] mb-4 overflow-hidden">
        ...
        {product.badge && (
          <span className="absolute top-4 right-4 bg-gray-900 text-white py-1 px-3 text-[0.7rem] font-semibold rounded-full">
            {product.badge}
          </span>
        )}
        <img src={product.images[0]} alt={product.name} className="w-full h-full object-contain p-6 block" />
      </div>
...
function Hair() {
  ...
  const { categories, steps, types, concerns, ingredients, availability } = filtersData.hair;
  ...
  return (
    <div className="min-h-screen bg-white">
      <div
        ref={headerRef}
        className={`py-8 border-b border-gray-200 text-center transition-all duration-700 ${
          isHeaderVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
        }`}
      >
        <div className="max-w-7xl mx-auto px-5">
          <h1 className="text-3xl font-bold text-gray-900">Hair</h1>
