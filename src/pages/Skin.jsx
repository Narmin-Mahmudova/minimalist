...
import productsData from '../data/skin.json';
import useScrollAnimation from '../hooks/useScrollAnimation';
import { useCart } from '../context/CartContext';
import { useWishlist } from '../context/WishlistContext';

function ProductCard({ product, index, isInWishlist, addToWishlist, addToCart, navigate, getSelectedSize, handleSizeSelect }) {
    ...
    >
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex items-center gap-2.5 mb-3">
          <span className="text-base font-semibold text-gray-900">₹ {getSelectedSize(product)?.price ?? product.price}</span>
          <span className="text-sm text-gray-400 line-through">₹ {getSelectedSize(product)?.oldPrice ?? product.oldPrice}</span>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {product.sizes?.map(size => (
            <button
              key={size.label}
              className={`py-1 px-4 border text-sm cursor-pointer transition-all duration-200 ${
                getSelectedSize(product)?.label === size.label
...
function Skin() {
  ...
  const addToCart = (product) => {
    const selected = getSelectedSize(product);
    const finalPrice = selected ? selected.price : product.price;
    const finalOldPrice = selected ? selected.oldPrice : product.oldPrice;
    const sizeLabel = selected ? selected.label : null;

    addToCartCtx(product, 1, sizeLabel, finalPrice, finalOldPrice);
    navigate('/cart');
  };
  ...
  const { categories, steps, types, concerns, ingredients, availability } = filtersData.skin;
  ...
  const getPrice = (product) => getSelectedSize(product)?.price ?? product.price ?? 0;
    ...
    <div className="min-h-screen bg-white">
      ...
      >
        <div className="max-w-7xl mx-auto px-5">
          <h1 className="text-3xl font-bold text-gray-900">Skin</h1>
          <p className="text-base text-gray-500 max-w-3xl mx-auto mt-2 leading-relaxed">
